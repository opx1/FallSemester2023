...
#Constraining controls to joints
...
#Seperate script here
#Creating broken controls for joints


import maya.cmds as cmds

def create_broken_controls_for_joint(joint_name, parent_control=None):
    # Create NURBS curve control if it doesn't exist already
    control_name = joint_name.replace("_jnt", "_ctrl")
    if not cmds.objExists(control_name):
        # Create control
    ...
    children_joints = cmds.listRelatives(joint_name, children=True, type='joint') or []
    for child_joint in children_joints:
        create_broken_controls_for_joint(child_joint, control)

        


def create_broken_joint_controls():
    # Get selected joints
    selected_joints = cmds.ls(selection=True, type='joint')
    ...
    for joint_name in selected_joints:
        create_broken_controls_for_joint(joint_name)


# Call the function
create_broken_joint_controls()
